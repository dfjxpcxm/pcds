...
function dhtmlXGridObject(id) {
    ...
    var t_creator = function(name) {
        ...
        t.style.cssText = 'width:100%;table-layout:fixed;padding-right:20px;margin-right:20px;';
    ...
    this.Jb = false;
    this.Jr = false;
    this.setSerializationLevel = function(userData, QT, config, GM, GA, MZ) {
        this.Jb = userData;
        this.Jr = QT;
        this.Ld = config;
        this._chAttr = GM;
    ...
    this.xO = function(r, i) {
        ...
        out.push(" id='" + r.idd + "'");
        if ((this.Jr) && this.selectedRows._dhx_find(r) != -1)
            out.push(" selected='1'");
        ...
        out.push(">");
        if (this.Jb && this.UserData[r.idd]) {
            vU = this.UserData[r.idd].getKeys();
            for (var ii = 0; ii < vU.length; ii++)
                out.push("<userdata name='" + vU[ii] + "'>" + (this.LB ? "<![CDATA[" : "") + this.UserData[r.idd].get(vU[ii]) + (this.LB ? "]]>" : "") + "</userdata>")
        };
        var changeFl = false;
    ...
    this.serialize = function() {
        ...
        if (this.Jb && this.UserData["gridglobaluserdata"]) {
            var vU = this.UserData["gridglobaluserdata"].getKeys();
            for (var i = 0; i < vU.length; i++)
                out += "<userdata name='" + vU[i] + "'>" + this.UserData["gridglobaluserdata"].get(vU[i]) + "</userdata>"
        };
        if (this.Ld)
            out += this.Aj();
        out += this.JS();
...
dhtmlXGridObject.prototype = {
    ...
    wx : function(ind, state) {
        ...
        if ((this.hdr.rows[1]._childIndexes) && (this.hdr.rows[1]._childIndexes[ind] != ind))
            OC = this.hdr.rows[1]._childIndexes[ind];
        if (state == "none") {
    ...
    checkAll : function() {
        ...
        for (var cInd = 0; cInd < this.getColumnsNum(); cInd++) {
            if (this.getColType(cInd) == "ch")
                this.setCheckedRows(cInd, mode)
        }
    },
    uncheckAll : function() {
        this.checkAll(0)
    ...
    _drawTooltip : function(e) {
        ...
        if (!ced || !ced.cell || !ced.cell._attrs)
            return;
        if (el._title)
            ced.cell.title = "";
        if (!ced.cell._attrs['title'])
            el._title = true;
        if (ced)
            el.title = ced.cell._attrs['title'] || (ced.getTitle ? ced.getTitle() : (ced.getValue() || "").toString().replace(/<[^>]*>/gi, ""));
        return true
    },
    ...
    setCellExcellType : function(rowId, cellIndex, type) {
        this.tW(this.rowsAr[rowId], cellIndex, type)
    },
    ...
    setColumnExcellType : function(colIndex, type) {
        for (var i = 0; i < this.rowsBuffer.length; i++)
            if (this.rowsBuffer[i] && this.rowsBuffer[i].tagName == "TR")
                this.tW(this.rowsBuffer[i], colIndex, type);
    ...
    enableAutoWidth : function(mode, max_limit, min_limit) {
        this._awdth = [convertStringToBoolean(mode), parseInt(max_limit || 99999), parseInt(min_limit || 0)]
    },
        ...
        k40_0_0 : function() {
            ...
            if (this.editor && this.editor.combo)
                ...
                if (this.rowsBuffer[rowInd]) {
                    ...
                    if (master.pagingOn)
                        master.showRow(master.getRowId(rowInd))
                } else {
        ...
        k38_0_0 : function() {
            ...
            if (this.editor && this.editor.combo)
                ...
                if (rowInd != -1 && (!this.pagingOn || (rowInd != 1))) {
                    ...
                    if (master.pagingOn && nrow)
                        master.showRow(nrow.idd)
                } else {
    ...
    deleteRow : function(row_id, node) {
        ...
        if (this.pagingOn)
            this.changePage();
        if (!this._realfake)
            this.callEvent("onAfterRowDeleted", [row_id, pid]);
        this.callEvent("onGridReconstructed", []);
        if (this._ahgr)
            this.setSizes();
        return true
    ...
    _addRow : function(new_id, text, ind) {
        ...
        if (this._fillers) {
            ...
            this.rowsBuffer._dhx_insertAt(ind, row);
            if (this._fake)
                this._fake.rowsCol._dhx_insertAt(ind, null);
            this.rowsAr[row.idd] = row;
            var found = false;
            for (var i = 0; i < this._fillers.length; i++) {
                var f = this._fillers[i];
