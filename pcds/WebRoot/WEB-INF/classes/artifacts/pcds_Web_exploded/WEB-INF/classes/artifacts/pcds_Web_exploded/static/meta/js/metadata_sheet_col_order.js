	...
	/*column_ds.load({
		...
		callback:function(){
			var index = column_ds.findBy(function(r){
				if(r.get('xls_col_id')== metadata_id){
					return true;
				}else{
					return false;
				}
...
function updateDisOrder(store){
	...
	if(store&&store.getCount()>0){
		for ( var int = 0; int < store.getCount(); int++) {
			...
			params +=record.get("xls_col_id")+','+(store.indexOf(record)+1)+';';
		}
	}
//	var metadata_id = Ext.getCmp('metadata_id').getValue();
	...
	Ext.Ajax.request({
		   url:  pathUrl + '/metadata/xlscolumn/updateDisOrder',
