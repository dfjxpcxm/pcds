...
function doAddResource(rootID) {
	var addWindow = new Ext.Window({
		...
		items : [{
			...
			baseCls : 'x-plain',
			labelWidth : 85,
			url :pathUrl+ '/sysResource/addResource',
			items : [{
				xtype : 'textfield',
				fieldLabel : '上级资源代码',
				readOnly : true,
	...
	var div = Ext.getDom('icon_name').parentNode;
	var span1 = document.createElement("span");
	span1.style.marginLeft=5;
	
	span1.className = "span_left";
	span1.innerHTML = "<a href='javascript:showChooseWin()'><img src="+ pathUrl + "/public/images/icons/add.png></a>";
...
function doEditResource(tree, id) {
	var editWindow = new Ext.Window({
		...
		items : [{
			...
			baseCls : 'x-plain',
			labelWidth : 75,
			url : pathUrl+ '/sysResource/updateResource',
	...
	p.form.load({
		url : pathUrl + '/sysResource/getResourceById',
		params : {
			resource_id : id
		}
	});
...
function doDeleteResource(tree, id) {
	Ext.Ajax.request({
		url : pathUrl + '/sysResource/removeResource',
		params : {
			resource_id : id
		},
		method : 'POST',
...
function reloadResource() {
	Ext.Ajax.request({
		url : pathUrl + '/sysResource/reloadResource',
		params : {
			resource_id : id
		},
		method : 'POST',
