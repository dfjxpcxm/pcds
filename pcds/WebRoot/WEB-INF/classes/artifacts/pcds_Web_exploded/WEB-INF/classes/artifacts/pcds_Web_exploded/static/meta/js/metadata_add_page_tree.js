...
function showConfigPageTableTree(){
	...
	var root=getConfigTableRootNode('10','数据字典',expandConfigTableMetadataNode);
	var formItemAnchor = '91%';
	var initPanel = new Ext.tree.TreePanel({
		collapsible : false,
		id : 'pageMetadataTreePanel',
		frame : false,
	...
	configPageTableWindow = new Ext.Window({
		...
		buttons : [{
			text : '确定',
			handler : function() {
				var sm = Ext.getCmp('pageMetadataTreePanel').getSelectionModel();
 ...
 */
function getConfigTableRootNode(nodeId, nodeName, expandFn){
 ...
 */
function expandConfigTableMetadataNode(parentNode) {
			...
			success : function(response, options) {
				...
				if (tl) {
					for (var i = 0; i < tl.length; i++) {
						...
						if(!leaf) {
							cnode.on('expand', expandConfigTableMetadataNode);
						}
