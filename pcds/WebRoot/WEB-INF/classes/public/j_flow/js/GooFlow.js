...
//初始化区域图的对象
	...
	this.$type="";//当前被选定的结点/转换线类型,如果没选中或者工作区被清空,则为""
	...
	if(property.haveHead){
		...
		+"><label title='"+(property.initLabelText||"新流程_1")+"' "
		+(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"新流程_1")+"</label>";
		...
		if(property.headBtns){
			this.$head.on("click",{inthis:this},function(e){
				...
				switch($(tar).attr("class")){
					case "ico_new":		if(This.onBtnNewClick!=null)	This.onBtnNewClick();break;break;
					...
					case "ico_grant"	:if(This.grantUser!=null) This.grantUser();break;//if(This.onFreshClick!=null)	This.onFreshClick();break;
				}
			});
		}
		
		//标题双击事件定义
		this.$head.children("label").on('dblclick',{inthis:this},function(e){//参考 879
//			doTitleDblClick();
			var oldTxt=this.innerHTML;
			var This=e.data.inthis;
			var title = This.$head.children("label");
//			var t=getElCoordinate(title);
			This.$titleTextArea.val(oldTxt).css(
			...
			This.$workArea.parent().one("mousedown",function(e){
				...
				if(This.$titleTextArea.val().trim()){
					This.setTitle(This.$titleTextArea.val())
				}
				This.$titleTextArea.val("").hide();
			});
	...
	this.$workArea=$("<div class='GooFlow_work_inner' style='width:"+(width-20)+"px;height:"+(height-20)+"px'></div>")
	...
	if(this.$editable){
	  ...
	  this.$titleTextArea=$("<textarea></textarea>");//标题
	  this.$bgDiv.append(this.$titleTextArea);
	...
	loadDataAjax:function(para){
		...
		$.ajax({
			...
			error: function(XMLHttpRequest, textStatus, errorThrown){
				if(para.failure)	para.error(textStatus,errorThrown);
			}
		});
	},
...
//		var jsonData = JSON.stringify(This.exportData());
//		var This = this;
//		$.ajax({
//			url:para.url,
//			dataType:"json",
