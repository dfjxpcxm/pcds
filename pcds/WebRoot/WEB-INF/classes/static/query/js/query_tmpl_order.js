...
var column_ds = new Ext.data.JsonStore({
	...
	fields : ['ds_id','field_id','field_label']
});
		...
		buttons:[
		      {
		    	  text:'确定',
		    	  handler:function(){
		    		  updateDisOrder(column_ds,node.id);
		    		  orderWin.destroy();
		    	  }
		      },{
		    	  text:'取消',
...
function updateDisOrder(store,id){
	...
	if(store&&store.getCount()>0){
		for ( var int = 0; int < store.getCount(); int++) {
			...
			params +=record.get("ds_id")+','+record.get("field_id")+','+(store.indexOf(record)+1)+';';
		}
	}

	//field.setValue(index+1);
	Ext.Ajax.request({
		   url:  pathUrl + '/queryTmplMeta/updateOrder',
