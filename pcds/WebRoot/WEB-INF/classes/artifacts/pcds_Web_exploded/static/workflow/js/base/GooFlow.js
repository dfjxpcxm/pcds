...
//初始化区域图的对象
	...
	this.$offsetX = property.offsetX||0;//x轴偏移量
	this.$offsetY = property.offsetY||0;//y轴偏移量
	var headHeight=0;
	var tmp="";
	if(property.haveHead){
		...
		+"><label title='"+(property.initLabelText||"workflow")+"' "
		+(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"workflow")+"</label>";
	...
	if(this.$editable){
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		This.addNode(This.$id+"_node_"+This.$max,{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType,Vmodel:'00',Vvalue:'',Vcheck:false});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",width:120,height:14,
				left:t.left+x-This.$workArea[0].parentNode.scrollLeft-This.$offsetX,
				top:t.top+y-This.$workArea[0].parentNode.scrollTop-This.$offsetY}).data("id",This.$focus).focus();
	...
	initGroup:function(width,height){
	  ...
	  this.$group.on("dblclick",{inthis:this},function(e){
		...
		This.$textArea.val(oldTxt).css({display:"block",width:100,height:14,
			left:t.left+x-This.$workArea[0].parentNode.scrollLeft-This.$offsetX,
			top:t.top+y-This.$workArea[0].parentNode.scrollTop-This.$offsetY}).data("id",p.id).focus();
			This.$workArea.parent().one("mousedown",function(e){
			if(e.button==2)return false;
			if(This.$textArea.css("display")=="block"){
				This.setName(This.$textArea.data("id"),This.$textArea.val(),"area");
	...
	//增加一个流程结点,传参为一个JSON,有id,name,top,left,width,height,type(结点类型),Vmodel,Vvalue,Vcheck等属性
...
//		}else{
			json.width=50;json.height=50;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px;width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><table cellspacing='0'><tr><td class='ico'><i class='node_"+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span' Vmodel='"+json.Vmodel+"' Vvalue='"+json.Vvalue+"' Vcheck='"+json.Vcheck+"'>"+json.name+"</div></div>");
//		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			document.onmousemove=function(e){
				...
				//偏移图标
				This.$ghost.css({left:X-This.$offsetX+hack+"px",top:Y-This.$offsetY+hack+"px"});
				isMove=true;
			}
			document.onmouseup=function(e){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:100,
				left:t.left+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft-This.$offsetX-24,
				top:t.top+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop-This.$offsetY+26})
		...
		//yangmou新增绑定图标双击功能添加判断属性 
		this.$workArea.delegate(".ico","dblclick",{inthis:this},function(e){
			var nodeObj = $(this).parents(".GooFlow_item");
...
//			}
	...
	//
	setParams:function(id,name,type,Vmodel,Vvalue,Vcheck){
		var oldName;
		if(type=="node"){//如果是结点
			if(!this.$nodeData[id])	return;
			if(this.onItemRename!=null&&!this.onItemRename(id,name,"node"))	return;
			oldName=this.$nodeData[id].name;
			this.$nodeData[id].name=name;
			this.$nodeData[id].Vmodel=Vmodel;
			this.$nodeData[id].Vvalue=Vvalue;
			this.$nodeData[id].Vcheck=Vcheck;
//			if(this.$nodeData[id].type.indexOf("round")>1){
				this.$nodeDom[id].children(".span").text(name);
//			}
			if(this.$editable){
	...
	getNextNode:function(id){
		...
		var num = 0;
		for(var k in this.$lineData){
			if(this.$lineData[k].from==id){
				var arr2 = new Array();
				var nodeId = this.$lineData[k].to;
	...
	getNextLine:function(id){
		...
		for(var k in this.$lineData){
			if(this.$lineData[k].from==id){
				var arr2 = new Array();
				var _curr_line = this.$lineData[k];
				var _link_to = _curr_line.to;
				var _link_name = _curr_line.name;
	...
	exportData:function(){
		...
		//校验开始节点
		for(var S1 in ret.nodes){
			if(ret.nodes[S1].type == 'start'){
				start_num ++;
				startNode = S1;
			}else if(ret.nodes[S1].type == 'end'){
				end_num ++;
				var end_Line = 0;  //连接到结束节点的线
				for(var L1 in ret.lines){
					if(ret.lines[L1].from == S1){
						alert("结束节点"+S1+"不能有连出线");
						return false;
					}
					if(ret.lines[L1].to == S1){
						end_Line ++;
					}
				}
			if(end_Line == 0){
			...
			}else if(ret.nodes[S1].type == 'fork'){
				var fork_num_f = 0;
				var fork_num_t = 0;
				for(var L1 in ret.lines){
					if(ret.lines[L1].from == S1){
				...
				if(fork_num_t != 2 ){
					alert("一分二节点"+S1+"只能有两条连出线");
					return false;
				}
				if(fork_num_f != 1 ){
					alert("一分二节点"+S1+"只能有一条连入线");
					return false;
				}
			}else if(ret.nodes[S1].type == 'task'){
				var task_num_t = 0;
				var task_num_f = 0;
				for(var L1 in ret.lines){
					if(ret.lines[L1].from == S1){
				...
				if(task_num_t != 1 ){
					alert("人机节点"+S1+"只能有一条连出线");
					return false;
				}
				if(task_num_f == 0 ){
					alert("人机节点"+S1+"必须至少有一条连入线");
					return false;
				}
			}
		}
		...
		if(start_num > 1 ){
			alert("流程校验失败，开始节点只能有一个");
			return false;
		}else if(start_num <= 0){
			alert("流程校验失败，必须有一个开始节点");
			return false;
		}else{
			var num = 0;
			for(var L1 in ret.lines){
				if(ret.lines[L1].from == startNode){
					num ++;
				}
				if(ret.lines[L1].to == startNode){
					alert("开始节点"+startNode+"不能有连入线");
					return false;
				}
			}
			if(num > 1 ){
		...
		for(var k1 in ret.nodes){
			...
			for(var k2 in ret.lines){
				if(ret.lines[k2].from == k1 || ret.lines[k2].to == k1){
					b = false;
				}
			}
			if(b){
